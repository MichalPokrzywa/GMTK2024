...
namespace Verpha.HierarchyDesigner
{
    ...
    public class HierarchyDesigner_Visual_Tools
    {
        ...
        private static GUIStyle _lockLabelStyle;
        private static GUIStyle lockLabelStyle
        {
            get
            {
                if (_lockLabelStyle == null)
                {
        ...
        {
            gameObjectCache = new Dictionary<int, GameObject>();
            foreach (var kvp in gameObjectCache)
            {
                GameObject obj = EditorUtility.InstanceIDToObject(kvp.Key) as GameObject;
                if (obj != null)
                {
                    gameObjectCache[kvp.Key] = obj;
                }
            }
