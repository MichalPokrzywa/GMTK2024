...
namespace Verpha.HierarchyDesigner
{
    ...
    public static class HierarchyDesigner_Visual_GameObject
    {
        ...
        {
            var deadComponentReferences = new List<WeakReference>();
            ...
            foreach (var deadRef in deadComponentReferences)
            {
                componentCache.Remove(deadRef);
            }

            var keysToRemove = new List<int>();
            foreach (var key in branchIconCache.Keys)
            {
        ...
        {
            if (!HierarchyDesigner_Manager_Settings.ShowMainIconOfGameObject || ShouldSkipDrawing(gameObject) || gameObject == null || HierarchyDesigner_Visual_Folder.IsFolder(gameObject)) return;
        ...
        private static Texture2D GetMainIconForGameObject(GameObject gameObject)
        {
            ...
            return mainIconCache.TryGetValue(instanceID, out Texture2D mainIcon) ? mainIcon : null;
        }
        ...
        private static Component FindMainUIComponent(Component[] components)
        {
            ...
            {
                if (c != null && c.GetType().ToString().Contains("TMP"))
                {
                    tmpComponent = c;
                    break;
                }
            ...
            {
                if (component != null &&
                    (component is Button || component is Scrollbar ||
                     component is Text || component is RawImage))
                {
                    return component;
                }
            }
            ...
            bool otherComponentsExist = false;
            foreach (Component c in components)
            {
                if (c != null && c is Image)
                {
                    imageComponent = c;
                }
                else if (c != null && (c is Button || c is Scrollbar || c is Text || c is RawImage || c.GetType().ToString().Contains("TMP")))
                {
                    otherComponentsExist = true;
                    break;
                }
            }

            if (imageComponent != null && !otherComponentsExist)
            {
        ...
        private static void CleanupComponentArrayPool()
        {
            ...
            foreach (var componentsArray in componentArrayPool)
            {
                bool hasNonNullComponent = false;
        ...
        {
            if (!HierarchyDesigner_Manager_Settings.ShowHierarchyTree || Event.current.type != EventType.Repaint || gameObject == null) return;

            const float iconOffset = 14f;
            const float initialOffset = 22f;

            Transform transform = gameObject.transform;
            Rect rect = new Rect(selectionRect.x - initialOffset, selectionRect.y, selectionRect.height, selectionRect.height);

            if (transform.parent != null)
            {
                Texture2D branchIcon = GetOrCreateBranchIcon(transform);
                GUI.DrawTexture(rect, branchIcon, ScaleMode.ScaleToFit, true, 0, TreeColor, 0, 0);
                DrawParentTreeLines(transform.parent, rect, iconOffset);
            }
        }
        ...
        private static void DrawParentTreeLines(Transform parentTransform, Rect rect, float iconOffset)
        {
            Color currentTreeColor = GetCachedTreeColor();
